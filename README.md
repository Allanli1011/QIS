...
src/qis/
  ...
  data/curve.py     多月合约曲线（c1..c4）加载与整理
...
### ④ carry：测量问题已根治，但确认它没有 alpha

原始 `c1/c2 − 1` 的日度测量噪声让"carry 高"往往只是 c1 当天被推高、次日回落，
策略于是系统性追高（**corr(信噪比, IC) = −0.53**）。修了四步之后仍是 −0.39，
于是拉了整条远期曲线（c1..c4，`scripts/fetch_curve.py`）用**回归斜率**估 carry：

    log F_i = a + b·t_i ,  t_i = (i−1)·h    carry = −b

N 点回归斜率的方差约为两点差分的 1/N。实测（本池真实数据，年化 carry 的绝对日噪声）：

| 品种 | 两点 c1/c2 | 曲线 (1,2,3,4) | 曲线 (2,3,4) |
|---|---:|---:|---:|
| SILVER | 0.0598 | 0.0274 | **0.0167** |
| GOLD | 0.0283 | 0.0114 | **0.0075** |
| IRONORE | 0.1245 | 0.0547 | **0.0347** |

中位降噪 2.2×（去掉近月后 3~4×）。近月要去掉：c1 临近到期有收敛与流动性效应，
是曲线上噪声最大的一点。**但金融期货相反**（SP500/BUND 的 c1/c2 本来就干净、
c3/c4 更薄），所以实现里按可用月份数取舍：≥4 个月用 (2,3,4)，3 个月用 (1,2,3)，
不够的回退 c1/c2。曲线深度：62/63 个品种有 ≥3 个月份，53 个有 4 个月份。

**三项独立证据表明测量问题已经解决：**

    绝对日噪声      降低 2~4 倍
    方向 IC        −0.023 → +0.0027（56% 的标的为正，负偏彻底消除）
    费前 Sharpe    −0.32 → −0.08（随测量变干净单调上升）

**然后它停在了零。** 费前 Sharpe −0.08、IC +0.003 都是噪声量级；
费后 −0.20 基本就是成本（拖累 0.06）。也就是说：把 carry 测准之后，
本池 2010 年以来**没有可提取的期限结构 alpha**——此前的 −2.5 完全是测量假象，
而不是一个"做反了、翻过来就能赚"的信号。

**两条后续路线都试过了，都不成立：**

*按品种分层.* 经济论证是成立的——各类别的 carry 机制完全不同，
且离散度差 50 倍（能源 0.310 vs 利率 0.006），混在一个截面里排名时
能源和农产品完全主导、债券利率永远拿不到信号。但组内排名 + 组内标准化
（等风险）之后反而更差：混合截面 −0.19、分层 −0.44、分层等风险 −0.46。
各类别单独跑也全是零或负（bond +0.09、equity +0.02、energy −0.48、ags −0.24）。
分组变小让信号更噪，而每一类本身也没有可提取的预测力。

*当风险因子做对冲.* 这个方向发现了一件真事：**trend 与 xsmom 都长期背着
正的 carry 因子暴露**（均值 +0.20 / +0.17，77% / 71% 的交易日为正），
而这两个策略没有任何一处显式表达过对 carry 的看法——是无意中的因子押注。
但把它中性化掉并不是稳健的改进：
...
全样本看上去"最大回撤 −19.8% → −14.4%、Calmar +39%"很诱人，但那**完全来自
2018-2021 一段**（含新冠）。最大回撤按定义由最坏的单次事件决定，很容易被一次
行情带偏——分段一看就现原形。唯一单调的效果是收益偏度改善（trend −0.41 → −0.17）。
按 carry 方向倾斜（加码/减码一致的持仓）则任何强度、任何符号都更差，
与"carry 没有 alpha"完全一致。
